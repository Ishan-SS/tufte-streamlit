...
# Create a directory for uploaded data
...
if openai_key:
    ...
    if upload_own_data:
        ...
        st.session_state.selected_dataset = next(d["url"] for d in datasets if d["label"] == selected_dataset_label)
...
if openai_key and st.session_state.get('selected_dataset'):
    ...
    # Create Orchestrator instance if needed
    if st.session_state.orc is None or st.session_state.last_dataset != st.session_state.selected_dataset:
    ...
    # Generate goals if not already generated
    ...
    if (st.session_state.visualizations is None or 
        st.session_state.last_goal_index != st.session_state.selected_goal_index or
